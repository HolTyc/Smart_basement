...
void napsaniDatSensoru() {
  String sensorZpravy[7] = {"Teplota: ", "Vlhkost: ", "Tlak: ", "c[CO]: ", "c[CH4]: ", "c[CO2]: "};
  String jednotky[7] = {"C", "%", "hPa", "PPM", "PPM", "PPM"};
  if (decodedKey-'0'==3) {
	sensorData = sensorData * 10;
  } else if (decodedKey-'0'==6) {
	sensorData = sensorData * 10;
  }
  if (decodedKey - '0' >= 1 and decodedKey - '0' <= 6) {
	lcd.clear();
	...
	if (decodedKey-'0'==4) {
		lcd.setCursor(0,1);
		lcd.print("Maximum 100PPM");
		lcd.setCursor(0,0);
	} else if (decodedKey-'0'==5) {
		lcd.setCursor(0,1);
		lcd.print("Maximum 1000PPM");
		lcd.setCursor(0,0);
	} else if (decodedKey-'0'==6) {
		lcd.setCursor(0,1);
		lcd.print("Maximum 5000PPM");
		lcd.setCursor(0,0);
	}
  }
  aktivaceSensor = 0;
